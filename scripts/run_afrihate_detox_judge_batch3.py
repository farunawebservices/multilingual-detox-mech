...
RESULTS = OUT / "afrihate_gpt5_gpt4_judged_batch3.csv"
PARTIAL = OUT / "afrihate_gpt5_gpt4_judged_batch3_partial.csv"
MANIFEST = OUT / "afrihate_gpt5_gpt4_judged_batch3_manifest.json"
...
def api_text(model, prompt, max_tokens):
    headers = {
        ...
        "X-Title": "afrihate-detox-gpt5-gpt4-judge-batch3",
    }
...
def collect_used_ids():
    used = set()
    for path in [PREV_SHORTLIST, PREV_21, PREV_FULL, PREV_BATCH2]:
        if not path.exists():
            continue
        df = pd.read_csv(path)
        if "id" in df.columns:

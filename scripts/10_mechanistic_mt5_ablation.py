#!/usr/bin/env python3
"""
Stage 10 Part B: Causal ablation on mT5 decoder cross-attention heads.
"""
import json
...
class Config:
    ...
    output_dir = "results/mechanistic/mt5_ablation"
    ablation_layers = [6, 8]
...
def ablate_cross_attention_heads(model, layer_idx, head_indices, n_heads=12):
    ...
    d_model = cross_attn.q.weight.shape[1]  # Should be 768 for mT5-base
    head_dim = d_model // n_heads  # 768/12 = 64
    
    # Get current weights
    W_q = cross_attn.q.weight.data  # (d_model, d_model) = (768, 768)
    W_k = cross_attn.k.weight.data
    W_v = cross_attn.v.weight.data
    W_o = cross_attn.o.weight.data  # (d_model, d_model) = (768, 768)
    
    # Compute mean across all heads for each weight matrix
    # Reshape to (n_heads, head_dim, d_model) then mean over heads
    ...
    mean_q = W_q_reshaped.mean(dim=0, keepdim=True)  # (1, head_dim, d_model)
    mean_k = W_k_reshaped.mean(dim=0, keepdim=True)
    mean_v = W_v_reshaped.mean(dim=0, keepdim=True)
    mean_o = W_o_reshaped.mean(dim=1, keepdim=True)  # (d_model, 1, head_dim)
    
    # Replace selected heads
...
def main():
    ...
    ablation_config = {layer: head_importance[layer][:config.n_heads_to_ablate] 
                       for layer in config.ablation_layers}
    ...
    results = {
        ...
        "n_heads_ablated": config.n_heads_to_ablate,
        "head_importance": {str(k): v.tolist() for k, v in head_importance.items()},
        "baseline": {
            "copy_rate": float(baseline_metrics["is_copy"].mean()),
            "sim": float(baseline_metrics["sim"].mean()),
            "fl": float(baseline_metrics["fl"].mean()),
            "j": float(baseline_metrics["j"].mean()),
        },
        "ablation": {
            "copy_rate": float(ablation_metrics["is_copy"].mean()),
            "sim": float(ablation_metrics["sim"].mean()),
            "fl": float(ablation_metrics["fl"].mean()),
            "j": float(ablation_metrics["j"].mean()),
        },
        "delta": delta,
    }

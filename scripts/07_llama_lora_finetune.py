#!/usr/bin/env python3
"""
Stage 7: Llama-3.2-3B-Instruct LoRA fine-tuning for multilingual detoxification.
Uses the frozen mT5 split for seed 2024 and output-only supervised loss.
"""
import json
import math
import random
...
import pandas as pd
import torch
_torch = torch  # Alias to avoid scoping issues in nested classes
from torch.utils.data import Dataset
from transformers import AutoTokenizer, AutoModelForCausalLM, BitsAndBytesConfig, TrainingArguments, Trainer
from peft import LoraConfig, get_peft_model, prepare_model_for_kbit_training, PeftModel, TaskType


class QLoRASafeTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        # Cast to FP32 for loss computation to avoid NaN in eval with QLoRA
        with __torch.cuda.amp.autocast(dtype=__torch.float32):
            return super().compute_loss(model, inputs, return_outputs=return_outputs, num_items_in_batch=num_items_in_batch)

class Config:
    model_id = "meta-llama/Meta-Llama-3-8B-Instruct"
    train_path = "results/training_v2_400plus/seed_2024/train.csv"
    dev_path = "results/training_v2_400plus/seed_2024/dev.csv"
    output_dir = "results/training_llama_lora/seed_2024"
    lora_r = 16
    lora_alpha = 32
    lora_dropout = 0.05
...
def set_seed(seed):
    random.seed(seed)
    np.random.seed(seed)
    __torch.manual_seed(seed)
    __torch.cuda.manual_seed_all(seed)
...
class DetoxDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Output-only supervised fine-tuning: do not train on system/user prompt.
...
def main():
    ...
    quantization_config = BitsAndBytesConfig(
        ...
        bnb_4bit_compute_dtype=__torch.bfloat16,
    )
    ...
    training_args = TrainingArguments(
        ...
        # greater_is_better=False,  # no eval
        bf16=False,
        fp16=False,
        report_to="none",
        seed=config.seed,
    ...
    import torch
    from __torch.utils.data import DataLoader
    from peft import PeftModel

    device = "cuda" if __torch.cuda.is_available() else "cpu"
    ...
    loader = DataLoader(dev_dataset, batch_size=4, shuffle=False, collate_fn=lambda xs: {
        "input_ids": __torch.stack([x["input_ids"] for x in xs]),
        "attention_mask": __torch.stack([x["attention_mask"] for x in xs]),
        "labels": __torch.stack([x["labels"] for x in xs]),
    })
    ...
    metadata = {
        ...
        "training": {
            ...
            "precision": "bf16",
            "objective": "output-only causal-LM loss",
        },

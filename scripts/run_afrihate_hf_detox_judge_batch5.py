...
RESULTS = OUT / "afrihate_hf_gpt5_gpt4_judged_batch5.csv"
PARTIAL = OUT / "afrihate_hf_gpt5_gpt4_judged_batch5_partial.csv"
MANIFEST = OUT / "afrihate_hf_gpt5_gpt4_judged_batch5_manifest.json"
...
TARGET_PER_LANG = 200  # 200 Yorùbá + 200 isiXhosa
...
def api_text(model, prompt, max_tokens):
    headers = {
        ...
        "X-Title": "afrihate-hf-detox-gpt5-gpt4-judge-batch5",
    }
...
def exclude_used_and_sample(yo_df, xh_df, used_ids, n_per_lang=TARGET_PER_LANG):
    ...
    print(f"Yorùbá rows sampled for batch5: {len(yo_filt)}", flush=True)
    print(f"isiXhosa rows sampled for batch5: {len(xh_filt)}", flush=True)
...
def main():
    ...
    manifest = {
        ...
        "pipeline": [
            ...
            "exclude all previously processed IDs (batches 1–4 + shortlist)",

...
INPUT = OUT / "afrihate_phase2_manual_shortlist_30.csv"

RESULTS = OUT / "afrihate_gpt5_gpt4_judged_21.csv"
PARTIAL = OUT / "afrihate_gpt5_gpt4_judged_21_partial.csv"
MANIFEST = OUT / "afrihate_gpt5_gpt4_judged_21_manifest.json"
LOG = OUT / "afrihate_gpt5_gpt4_judged_21.log"
...
def api_text(model, prompt, max_tokens):
    headers = {
        ...
        "X-Title": "afrihate-detox-gpt5-gpt4-judge",
    }
...
def main():
    source_df = pd.read_csv(INPUT)

    if "manual_shortlist_id" in source_df.columns:
        source_df["run_id"] = source_df["manual_shortlist_id"].astype(int)
    else:
        source_df["run_id"] = range(1, len(source_df) + 1)
    ...
    print(f"Input rows: {len(source_df)}", flush=True)
    ...
    for _, item in source_df.iterrows():
        ...
        print(
            f"[{run_id}/{len(source_df)}] Generate | {language} | "
            f"{model} | {source[:90]}",
            flush=True,
        )
        ...
        print(
            f"[{run_id}/{len(source_df)}] Judge | {JUDGE_MODEL}",
            flush=True,
        )

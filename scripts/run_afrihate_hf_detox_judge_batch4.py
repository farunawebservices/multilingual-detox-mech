...
# Full AfriHate HF data
AFRIHATE_HF_YOR = ROOT / "data" / "external" / "afrihate_hf" / "yor_all_data.tsv"
AFRIHATE_HF_XHO = ROOT / "data" / "external" / "afrihate_hf" / "xho_all_data.tsv"
...
RESULTS = OUT / "afrihate_hf_gpt5_gpt4_judged_batch4.csv"
PARTIAL = OUT / "afrihate_hf_gpt5_gpt4_judged_batch4_partial.csv"
MANIFEST = OUT / "afrihate_hf_gpt5_gpt4_judged_batch4_manifest.json"
...
TARGET_PER_LANG = 600  # number of toxic rows to sample per language

if not KEY:
    raise SystemExit("OPENROUTER_API_KEY is not set.")
if not AFRIHATE_HF_YOR.exists() or not AFRIHATE_HF_XHO.exists():
    raise SystemExit("AfriHate HF TSV files missing.")
...
def api_text(model, prompt, max_tokens):
    headers = {
        ...
        "X-Title": "afrihate-hf-detox-gpt5-gpt4-judge-batch4",
    }
...
def load_afrihate_hf_toxic():
    ...
    # Load isiXhosa splits
    xh_train = pd.read_csv(HF_DIR / "xho_train.tsv", sep="\t")
    xh_dev   = pd.read_csv(HF_DIR / "xho_dev.tsv", sep="\t")
    xh_test  = pd.read_csv(HF_DIR / "xho_test.tsv", sep="\t")

    xh = pd.concat([xh_train, xh_dev, xh_test], ignore_index=True)

    yo["language"] = "Yorùbá"
    xh["language"] = "isiXhosa"

    yo["source_dataset"] = "AfriHate_HF"
    xh["source_dataset"] = "AfriHate_HF"

    yo["source_config"] = "yor_splits"
    xh["source_config"] = "xho_splits"

    yo["source_split"] = "all"
    xh["source_split"] = "all"

    # Filter to toxic labels only
    toxic_labels = {"Abuse", "Hate"}
    yo = yo[yo["label"].isin(toxic_labels)].copy()
    xh = xh[xh["label"].isin(toxic_labels)].copy()

    # Ensure id is string
    yo["id"] = yo["id"].astype(str)
    xh["id"] = xh["id"].astype(str)

    yo["label"] = yo["label"].astype(str)
    xh["label"] = xh["label"].astype(str)

    yo["toxic_input_raw"] = yo["tweet"].astype(str)
    xh["toxic_input_raw"] = xh["tweet"].astype(str)
    ...
    xh["toxic_input_processing"] = xh["toxic_input_raw"].apply(normalize_text)
...
def exclude_used_and_sample(yo_df, xh_df, used_ids, n_per_lang=TARGET_PER_LANG):
    yo_filt = yo_df[~yo_df["id"].isin(used_ids)].copy()
    xh_filt = xh_df[~xh_df["id"].isin(used_ids)].copy()

    print(f"Yorùbá toxic rows after exclude: {len(yo_filt)}", flush=True)
    print(f"isiXhosa toxic rows after exclude: {len(xh_filt)}", flush=True)

    if len(yo_filt) > n_per_lang:
        yo_filt = yo_filt.sample(n=n_per_lang, random_state=42).reset_index(drop=True)
    if len(xh_filt) > n_per_lang:
        xh_filt = xh_filt.sample(n=n_per_lang, random_state=42).reset_index(drop=True)

    print(f"Yorùbá rows sampled for batch4: {len(yo_filt)}", flush=True)
    print(f"isiXhosa rows sampled for batch4: {len(xh_filt)}", flush=True)

    return yo_filt, xh_filt

def main():
    yo_pool, xh_pool = load_afrihate_hf_toxic()
    used_ids = collect_used_ids()

    print(f"Total used IDs so far: {len(used_ids)}", flush=True)

    yo_pool, xh_pool = exclude_used_and_sample(yo_pool, xh_pool, used_ids, TARGET_PER_LANG)
    ...
    manifest = {
        "created_utc": datetime.now(timezone.utc).isoformat(),
        "input_yoruba_hf": str(AFRIHATE_HF_YOR),
        "input_xhosa_hf": str(AFRIHATE_HF_XHO),
        "target_per_language": TARGET_PER_LANG,
        "input_yoruba_rows_sampled": int(len(yo_pool)),
        "input_xhosa_rows_sampled": int(len(xh_pool)),
        "generator_models": {"Yorùbá": YO_MODEL, "isiXhosa": XH_MODEL},
        "judge_model": JUDGE_MODEL,
        "pipeline": [
            "AfriHate HF full toxic pool (Abuse+Hate)",
            "exclude all previously processed IDs",
            "sample up to TARGET_PER_LANG per language",

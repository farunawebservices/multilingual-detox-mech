#!/usr/bin/env python3
"""Stage 4 v2: Generate and evaluate mT5 trained on 400+ pairs."""
import json
...
def main():
    ...
    dup = baseline_df.copy()
    dup["prediction"] = dup["toxic_input"]
    dup["pair_id"] = [f"{r['language']}_{i}" for i, r in dup.iterrows()]
    dup = dup[["pair_id", "language", "toxic_input", "detox_output", "prediction"]]
    dup.to_csv(out_dir / "predictions_duplicate.csv", index=False)
    ...
    for seed, pred_df in all_preds.items():
        ...
        pred_df["model"] = f"mt5_v2_seed_{seed}"
        all_dfs.append(pred_df)

    dup = pd.read_csv(out_dir / "predictions_duplicate.csv")
    print(f"  Duplicate baseline...")
    dup = compute_sim(dup, embedder)
    dup = compute_fluency(dup)
    dup = compute_copy_rate(dup)
    dup["sta"] = 1.0
    dup["j"] = (dup["sta"] + dup["sim"] + dup["fl"]) / 3.0
    dup["model"] = "duplicate"
    all_dfs.append(dup)

#!/usr/bin/env python3
"""Stage 5: Fine-tune mT0-base on full 400+ pairs corpus."""
# Identical to scripts/03_train_mt5_detox_v2_400plus.py except model_id
...
# [Rest is identical - copy the functions from scripts/03_train_mt5_detox_v2_400plus.py]
...
def make_splits(df, seed):
    ...
    for lang, group in df.groupby("language", sort=True):
        ...
        manifest.append({
            "language": lang, "rows_total": len(group), "unique_inputs_total": n_unique,
            "train_unique_inputs": len(train_inputs), "dev_unique_inputs": len(dev_inputs), "test_unique_inputs": len(test_inputs),
            "train_rows": len(train_g), "dev_rows": len(dev_g), "test_rows": len(test_g),
        })
    splits = {"train": pd.concat(train_parts, ignore_index=True), "dev": pd.concat(dev_parts, ignore_index=True), "test": pd.concat(test_parts, ignore_index=True)}
    ...
    return splits, pd.DataFrame(manifest)
...
class DetoxDataset(TorchDataset):
    ...
    def __getitem__(self, index):
        ...
        encoded_source = self.tokenizer(source, max_length=self.config.max_input_length, truncation=True, padding=False)
        encoded_target = self.tokenizer(row["detox_output"], max_length=self.config.max_target_length, truncation=True, padding=False)
        return {"input_ids": encoded_source["input_ids"], "attention_mask": encoded_source["attention_mask"], "labels": encoded_target["input_ids"]}
...
def train_seed(config, seed):
    ...
    steps_per_epoch = int(np.ceil(len(train_dataset) / (config.per_device_train_batch_size * config.gradient_accumulation_steps)))
    warmup_steps = max(1, int(steps_per_epoch * 0.10))
    print(f"\nSteps/epoch: {steps_per_epoch}; warmup steps: {warmup_steps}")
    args = TrainingArguments(
        output_dir=str(out_dir), do_train=True, do_eval=True, eval_strategy="steps", eval_steps=config.eval_steps,
        save_strategy="no", logging_steps=config.logging_steps, logging_first_step=True, num_train_epochs=config.num_train_epochs,
        ...
        gradient_accumulation_steps=config.gradient_accumulation_steps, learning_rate=config.learning_rate, weight_decay=config.weight_decay,
        warmup_steps=warmup_steps, lr_scheduler_type="linear", bf16=config.bf16, fp16=False, seed=seed, report_to="none",
        dataloader_num_workers=0, dataloader_pin_memory=True, optim="adamw_torch", remove_unused_columns=False,
    )
    trainer = Trainer(model=model, args=args, train_dataset=train_dataset, eval_dataset=dev_dataset, data_collator=collator)
    ...
    summary = {
        "seed": seed, "model_id": config.model_id, "split_rule": "grouped_by_toxic_input_70_15_15_all_rows",
        "train_samples": len(splits["train"]), "dev_samples": len(splits["dev"]), "test_samples": len(splits["test"]),
        "best_observed_eval_loss": best_eval_loss, "best_observed_eval_step": best_eval_step,
        "train_loss": float(result.training_loss), "global_step": int(result.global_step), "epochs_trained": float(trainer.state.epoch),
        "finite_final_parameters": bool(finite_parameters), "checkpoint_policy": "final_only_no_optimizer_checkpoint",
    }
...
def main():
    ...
    aggregate = {
        "model_id": config.model_id, "split_rule": "grouped_by_toxic_input_70_15_15_all_rows", "seeds": [42, 1337, 2024],
        "per_seed_summaries": summaries,
        "mean_train_loss": float(np.mean([x["train_loss"] for x in summaries])),
        "mean_best_observed_eval_loss": float(np.mean([x["best_observed_eval_loss"] for x in summaries if x["best_observed_eval_loss"] is not None])),
    }
    output = Path(config.output_dir)
    output.mkdir(parents=True, exist_ok=True)
    (output / "aggregate_summary.json").write_text(json.dumps(aggregate, indent=2, ensure_ascii=False), encoding="utf-8")
    print("\nALL SEEDS COMPLETE")

...
# Previously processed shortlist (21 items)
PREV_SHORTLIST = OUT / "afrihate_phase2_manual_shortlist_30.csv"

RESULTS = OUT / "afrihate_gpt5_gpt4_judged_full.csv"
PARTIAL = OUT / "afrihate_gpt5_gpt4_judged_full_partial.csv"
MANIFEST = OUT / "afrihate_gpt5_gpt4_judged_full_manifest.json"
LOG = OUT / "afrihate_gpt5_gpt4_judged_full.log"
...
EXAMPLES = {
    "Yorùbá": [
        (
            "Ìwọ jẹ́ aláìmọ̀.",
            "Ìwọ nílò lati kọ́ ẹ̀kọ́ síi."
        ),
        (
            "O jẹ́ asán.",
            "Ìṣe rẹ ní àǹfààní lati dara síi."
        ),
        (
            "Ìdáhùn rẹ̀ burú.",
            "Ìdáhùn rẹ̀ nílò ìtúnṣe díẹ̀."
        ),
    ],
    "isiXhosa": [
        (
            "Kufana nokuthetha nodonga lwezitena kunye nawe.",
            "Kunzima ukuthetha nawe."
        ),
        (
            "Udlala umdlalo oyingozi.",
            "Ndixhalabile ngomngcipheko obandakanyekayo."
        ),
        (
            "Umsindo wam uya kuba phezu kwakho.",
            "Ndifuna ukuthetha nawe ngomsindo wam."
        ),
    ],
}
...
def api_text(model, prompt, max_tokens):
    headers = {
        ...
        "X-Title": "afrihate-detox-gpt5-gpt4-judge-full",
    }
...
def load_afrihate_pools():
    ...
    # source_dataset, source_config, source_split, id, language, label,
    # length, toxic_input_raw, toxic_input_processing, existing_parallel_overlap, phase1_status

    # Use toxic_input_processing as the source text for generation/judging
    yo["source_text"] = yo["toxic_input_processing"].astype(str)
    xh["source_text"] = xh["toxic_input_processing"].astype(str)

    return yo, xh

def exclude_previous(yo_df, xh_df, prev_df):
    if prev_df is None or prev_df.empty:
        return yo_df.copy(), xh_df.copy()

    # prev_df uses 'id' column, same as AfriHate pools
    prev_ids = set(prev_df["id"].astype(str))

    yo_filt = yo_df[~yo_df["id"].astype(str).isin(prev_ids)].copy()
    xh_filt = xh_df[~xh_df["id"].astype(str).isin(prev_ids)].copy()
...
def main():
    ...
    prev_df = None
    if PREV_SHORTLIST.exists():
        prev_df = pd.read_csv(PREV_SHORTLIST)

    yo_pool, xh_pool = exclude_previous(yo_pool, xh_pool, prev_df)

    print(f"Yorùbá pool size after exclude: {len(yo_pool)}", flush=True)
    print(f"isiXhosa pool size after exclude: {len(xh_pool)}", flush=True)
    ...
    for _, item in combined.iterrows():
        ...
        generated, gen_finish, gen_error, gen_body = api_text(
            model=model,
            prompt=gen_prompt,
            max_tokens=2500,
        )
        ...
        print(
            f"[{run_id}/{len(combined)}] Judge | {JUDGE_MODEL}",
            flush=True,
        )
        ...
        judge_raw, judge_finish, judge_error, judge_body = api_text(
            model=JUDGE_MODEL,
            prompt=judge_prompt,
            max_tokens=700,
        )
        ...
        row = {
            ...
            "source_decision": get_value(
                judge_obj,
                "source_decision",
                "judge_error",
            ),
            "final_decision": get_value(
                judge_obj,
                "final_decision",
                "judge_error",
            ),
            "source_reason": get_value(judge_obj, "source_reason", ""),
            "toxicity_removed": get_value(
                judge_obj,
                "toxicity_removed",
                "",
            ),
            "residual_toxicity": get_value(
                judge_obj,
                "residual_toxicity",
                "",
            ),
            "meaning_preserved": get_value(
                judge_obj,
                "meaning_preserved",
                "",
            ),
            "naturalness": get_value(
                judge_obj,
                "naturalness",
                "",
            ),
            "language_correct": get_value(
                judge_obj,
                "language_correct",
                "",
            ),
            "code_switch_status": get_value(
                judge_obj,
                "code_switch_status",
                "",
            ),
            "new_content_added": get_value(
                judge_obj,
                "new_content_added",
                "",
            ),
            "generic_template": get_value(
                judge_obj,
                "generic_template",
                "",
            ),
            "judge_reason": get_value(judge_obj, "reason", ""),
            "human_review_required": get_value(
                judge_obj,
    ...
    manifest = {
        ...
        "input_yoruba_rows": int(len(yo_pool)),
        "input_xhosa_rows": int(len(xh_pool)),
        "generator_models": {
            "Yorùbá": YO_MODEL,
            "isiXhosa": XH_MODEL,
        },
        ...
        "pipeline": [
            ...
            "exclude previously processed shortlist",
